module.exports = (client, guild) => {
  ...
  if(!log_channel) return console.log(`LOL, stpid guys, your log channel already get delete ayyyyy`)
  log_channel.send(new client.discord.RichEmbed().setTitle(`Join server log`).setColor("GREEN").addField(`Guild name`,guild.name,true).addField(`Member count`,guild.memberCount,true).addField(`Server Count`,client.guilds.cache.size,true).addField(`Region`,region,true).addField(`Owner`,client.users.cache.get(guild.ownerID).tag,true).setImage(guild.iconURL({format: "jpg"})));
